...
$(document).ready(function () {
    sortableitems();
    $('#sortablenewProducts').prepend('<div id="ajaxresult"></div>');
});

function sortableitems() {
    $("#sortablenewProducts").sortable().bind('sortupdate', function () {
        var arrayitems = [];
        var iditem = '';

        $(this).find('li').each(function () {
            var current = $(this);
            iditem = current.children('div').data('id');
            arrayitems.push(iditem);
        ...
        $.getJSON(
            '../modules/ordernewproduct/ajax_updateproducts.php',
            {arrayitems: arrayitems}
        ).done(function (data) {
                // $('#sortablenewProducts #ajaxresult').text(data);
                if (data == 'ok') {
                    $.growl.notice({ title: "OK !", message: "L'ordre des nouveautés est mis à jour." });
                } else {
                    $.growl.error({ title: "Erreur !", message: "L'ordre des nouveautés n'est pas mis à jour." });
                }
            });
    });
}